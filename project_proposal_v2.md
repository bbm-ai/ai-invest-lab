...
* **自主運作**：無需人工干預，自動循環執行。
* **手機友好**：**Web 儀表板 (Streamlit)** 響應式設計，透過 **Telegram/Slack/Email** 推送關鍵報告與告警。
* **系統健壯性**：**API 故障切換 (Failover)** 機制，確保決策不中斷。
* **數據驅動**：所有決策可追溯、可回測，保留完整日誌與版本，所有報表可見性、每日、每周、每月、每年的回測績效評比。
* **初始資本**：以模擬交易為核心，資本為100萬美金，benchmark 為VOO (代表美股大盤績效)。
...
| **Phase 2** | **Day 8–14** | **AI 智能增強**：Groq/Gemini/GLM/Claude API、情感分析、技術分析、策略生成、**API 智能路由**。 | **Milestone 2**：多模態分析策略輸出，含成本日誌。 |
